...
const ImageFullscreenModal = ({ isOpen, images, selectedImageIndex, onClose, onNavigate }) => {
  ...
  const handlePrev = () => {
    onNavigate((validatedIndex - 1 + images.length) % images.length);
  };

  const handleNext = () => {
    onNavigate((validatedIndex + 1) % images.length);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90 p-4">
      ...
      {/* Navigation Arrows (rendered only if more than one image) */}
      {images.length > 1 && (
        <>
          <button
            onClick={handlePrev}
