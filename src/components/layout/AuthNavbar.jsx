...
export default function AuthNavbar() {
  ...
  useEffect(() => {
    if (session?.user) {
      setUser({
        ...
        // KEY FIX: Use image from session, or fallback to default.
        // Google logins provide a URL, credentials logins provide null.
        avatar: session.user.image || "https://gateway.pinata.cloud/ipfs/QmYkedcDzkvyCZbPtzmztQZ7uANVYFiqBXTJbERsJyfcQm",
      });
      // Store token and userId for legacy API calls that need it
  ...
  // Effect for fetching cart and notifications when user is logged in
  useEffect(() => {
  ...
  const handleLogout = () => {
    ...
    localStorage.clear(); // Clear any other local storage items
    signOut({ callbackUrl: "/" });
  };

  const handleSellClick = () => {
    if (user) {
      router.push("/sell"); // Or seller dashboard logic
    } else {
      router.push("/login");
    }
  };
        ...
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-6 text-sm text-gray-800">
            ...
            <div className="relative" ref={desktopCategoryRef}>
              ...
              >
                {/* SVG Icon */}
                <svg className="w-5 h-5 group-hover:text-orange-500" fill="currentColor" viewBox="0 0 20 20"><path d="M5.99829 4.75C5.99829 4.33579 6.33408 4 6.74829 4H17.2483C17.6625 4 17.9983 4.33579 17.9983 4.75C17.9983 5.16421 17.6625 5.5 17.2483 5.5H6.74829C6.33408 5.5 5.99829 5.16421 5.99829 4.75ZM5.99829 10C5.99829 9.58579 6.33408 9.25 6.74829 9.25H17.2483C17.6625 9.25 17.9983 9.58579 17.9983 10C17.9983 10.4142 17.6625 10.75 17.2483 10.75H6.74829C6.33408 10.75 5.99829 10.4142 5.99829 10ZM5.99829 15.25C5.99829 14.8358 6.33408 14.5 6.74829 14.5H17.2483C17.6625 14.5 17.9983 14.8358 17.9983 15.25C17.9983 15.6642 17.6625 16 17.2483 16H6.74829C6.33408 16 5.99829 15.6642 5.99829 15.25Z" /><path d="M1.98828 4.75C1.98828 4.19772 2.436 3.75 2.98828 3.75H2.99828C3.55057 3.75 3.99828 4.19772 3.99828 4.75V4.76C3.99828 5.31228 3.55057 5.76 2.99828 5.76H2.98828C2.436 5.76 1.98828 5.31228 1.98828 4.76V4.75Z" /><path d="M1.98828 15.25C1.98828 14.6977 2.436 14.25 2.98828 14.25H2.99828C3.55057 14.25 3.99828 14.6977 3.99828 15.25V15.26C3.99828 15.8123 3.55057 16.26 2.99828 16.26H2.98828C2.436 16.26 1.98828 15.8123 1.98828 15.26V15.25Z" /><path d="M1.98828 10C1.98828 9.44772 2.436 9 2.98828 9H2.99828C3.55057 9 3.99828 9.44772 3.99828 10V10.01C3.99828 10.5623 3.55057 11.01 2.99828 11.01H2.98828C2.436 11.01 1.98828 10.5623 1.98828 10.01V10Z" /></svg>
                <span className="group-hover:text-orange-500">All Categories</span>
              </button>
              {categoryOpen && (
                <div className="absolute z-50 mt-2 w-48 bg-white border rounded-xl shadow-lg py-2">
                  {dropdownCategories.map((cat) => (
                    <Link key={cat.key} href={`/category/${categoryMap[cat.key]}`} className="block px-4 py-2 hover:bg-gray-100 text-sm">
                      {cat.name}
                    </Link>
                  ))}
                </div>
          ...
          <div className="flex items-center gap-4 text-gray-700 text-sm">
            {!user ? (
              ...
              <>
                {/* Cart Icon */}
                {/* <Link href="/buy/payment" className="cursor-pointer hover:text-orange-500">
        ...
        <div className="flex flex-col lg:flex-row gap-3 w-full">
            ...
            <div className="block lg:hidden w-full space-y-2">
                <nav className="flex justify-center py-2 md:hidden lg:hidden gap-6 text-sm">
                    {topCategories.filter(cat => !cat.showOn).map(cat => (
                        <Link key={cat.key} href={`/category/${categoryMap[cat.key]}`} className="hover:text-orange-500">{cat.name}</Link>
                    ))}
                     <div className="relative" ref={mobileCategoryRef}>
                        {/* Mobile "All Categories" button and dropdown */}
                     </div>
                </nav>
                <LocationDropdown />
                <SearchBar />
            ...
            <div className="hidden lg:flex gap-3 w-full items-start">
                <div className="w-[66%]"><SearchBar /></div>
                <div className="w-[34%]"><LocationDropdown /></div>
            </div>
        </div>
